...
function setMarkers(_x, _x2) {
  return _setMarkers.apply(this, arguments);
}

function _setMarkers() {
  _setMarkers = _asyncToGenerator(
    /*#__PURE__*/ regeneratorRuntime.mark(function _callee2(map, geocoder) {
      var locations,
        categories,
        i,
        location,
  ...
  geocoder.geocode(
    ...
    function (results, status) {
      if (status == google.maps.GeocoderStatus.OK) {
        if (results[0]) {
          input.value = "".concat(results[0].formatted_address);
        }
      }
    }
  );
...
function initMap() {
  ...
  map = new google.maps.Map(document.getElementById("map"), {
    ...
    center: {
      lat: 41.72276088034398,
      lng: 44.75888040785368
    }
  });
...
var LinkRedirect = function LinkRedirect(response) {
  setTimeout(function () {
    ...
    var link = "https://www.google.com/maps/dir/?api=1&origin="
      .concat(origin.lat(), ",")
      .concat(origin.lng(), "&destination=")
      .concat(destination.lat(), ",")
      .concat(destination.lng(), "&travelMode=")
      .concat(travelMode, "&dir_action=navigate");
